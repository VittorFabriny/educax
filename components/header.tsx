...
export function Header() {
  return (
    <header className="w-full h-16 py-4 px-5 md:px-14 flex justify-between items-center bg[var(--color-bg)] border-b border-gray-200">
      <a href="#" className="flex items-end gap-2 text-2xl font-bold font-poppins">
        <BookOpen className="h-8 w-8 text-(--primary-color)" />
        <span>EducaX</span>
      </a>

      <div className="hidden md:flex gap-8 items-center text-sm font-medium text-(--color-text-muted)">
        <a href="#">Cursos</a>
        <a href="#">Sobre</a>
        <a href="#">Contato</a>
      </div>
